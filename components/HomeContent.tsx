    ...
    <div className="space-y-8">
      ...
      <ProMaxPanel className="bg-slate-950 p-8 text-white shadow-[0_30px_80px_-30px_rgba(15,23,42,0.8)] dark:border-white/10">
        ...
        {featuredProducts.length === 0 ? (
          <p className="mt-8 text-sm text-slate-300">Our showroom catalog is being added right now — check back shortly, or explore the full products page.</p>
        ) : (
            ...
            {featuredProducts.map((product) => (
              <div key={product.id} className="rounded-[1.2rem] border border-white/10 bg-white/10 p-4 backdrop-blur">
                {product.image ? (
                  ...
                  <div className="flex h-36 w-full items-center justify-center rounded-[0.8rem] bg-white/10 text-xs uppercase tracking-wide text-slate-300">
                    No image yet
                  </div>
                )}
